...
namespace OpenWeen.UWP.ViewModel.DetailPage
{
    public class WeiboRepostViewModel : WeiboDetailListBase<MessageModel>
    {
        ...
        {
            var list = (await Core.Api.Statuses.Repost.GetRepost(ID, max_id: WeiboList[WeiboList.Count - 1].ID)).Reposts;
            list.RemoveAt(0);
            return list;
        }
        ...
        {
            var item = await Core.Api.Statuses.Repost.GetRepost(ID);
